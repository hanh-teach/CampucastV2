...
export class WebRtcVadAdapter implements VoiceActivityDetector {
  ...
  constructor(onEvent?: SpeechEventHandler) {
    ...
    loadVadPromise.then((VadClass) => {
      if (VadClass) {
        ...
        console.log('[WebRtcVadAdapter] webrtc-vad-wasm library not loaded. Running in high-precision adaptive energy fallback mode.');
      }
    });
  }
  ...
  process(frame: Float32Array): void {
    ...
    if (this.vadEngine) {
      ...
      // High-precision adaptive energy fallback
      this.noiseFloor = this.noiseFloor * 0.96 + Math.min(this.noiseFloor, rms) * 0.04;
      const adaptiveThreshold = Math.max(0.018, this.noiseFloor * 2.2);
      isSpeechFrame = rms > adaptiveThreshold;
    }

    // Unified Event notification across WASM and Adaptive Energy engines
    if (isSpeechFrame) {
      this.speechCounter++;
      this.silenceCounter = 0;
      ...
      if (this.silenceCounter >= 24) { // Debounce speech end (24 consecutive quiet frames ~ 2.4 seconds)
        if (this.isSpeechActive) {
          this.isSpeechActive = false;
