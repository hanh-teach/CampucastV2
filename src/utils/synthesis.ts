...
export function getCohostVoice(primaryVoice: string): string {
  ...
  // Vietnamese Dual-Host Pairings (Sprint 3.3):
  // Host A: MC Minh (Nam - Giọng miền Bắc: vi-HN-male)
  // Host B: MC An (Nữ - Giọng miền Nam: vi-HCM-female)
  if (voice === "vi-hn" || voice === "vi-hn-male" || voice === "vi") {
    return "vi-HCM-female";
  }
  if (voice === "vi-hcm" || voice === "vi-hcm-female") {
    return "vi-HN-male";
  }
  if (voice === "vi-hn-female") return "vi-HCM-male";
  if (voice === "vi-hcm-male") return "vi-HN-female";
  
  return "vi-HCM-female"; // Default cohost voice
}
