...
let sharedTickAudioCtx: AudioContext | null = null;
...
function playTick() {
  try {
    ...
    if (!sharedTickAudioCtx || sharedTickAudioCtx.state === "closed") {
      sharedTickAudioCtx = new AudioContextClass();
    }
    const ctx = sharedTickAudioCtx;
...
export function useDrivingMode(
  ...
  const { isListening, interimTranscript, audioLevel, startListening, stopListening } = useSpeechRecognition({
    uiLanguage,
    lang: uiLanguage === "vi" ? "vi-VN" : "en-US",
    continuous: isContinuous,
    interimResults: true,
    autoRestart: isContinuous && preferences.isDrivingMode,
    onStart: () => {
      setMicError("");
    },
