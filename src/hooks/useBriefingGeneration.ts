...
import { getAmbientContext, buildAmbientPromptInstruction } from "../services/ambientContextService";
import { recordListenedNews } from "../services/companionMemoryService";
...
interface UseBriefingGenerationProps {
  ...
  saveNewBriefing: (briefing: SavedSummary) => Promise<void>;
...
export function useBriefingGeneration({
  ...
  const executeStage2 = async (researchPkg: ResearchPackage, prefs: any): Promise<EditorialDraft> => {
    ...
    try {
      const ambientContext = getAmbientContext(uiLanguage);
      const ambientPromptText = buildAmbientPromptInstruction(ambientContext, uiLanguage);
      ...
      const draft = await generateEditorialDraft(researchPkg, enrichedPrefs, getApiUrl);
      ...
      if (parsedPayload.suggestedTags && !parsedPayload.confirmedTags) {
        parsedPayload.confirmedTags = [...parsedPayload.suggestedTags];
      }
  ...
  const handleGenerateAudio = async (): Promise<SavedSummary | undefined> => {
    ...
    try {
      ...
      const newBriefing: SavedSummary = {
        ...
        tags: activePayload?.confirmedTags || activePayload?.suggestedTags || [preferences.tone || "informative", preferences.commuteType || "driving"],
      ...
      // Record listened news topics into companion memory (48h rolling window)
      if (activePayload?.chapters && Array.isArray(activePayload.chapters)) {
        const listenedItems = activePayload.chapters.map((ch: any) => ({
          title: ch.topic || newBriefing.payload?.title || "Story",
          topic: ch.topic
        }));
        recordListenedNews(listenedItems);
      }
      try {
