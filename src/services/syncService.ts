...
import { SavedSummary, VoiceHistoryItem, SyncHistoryEvent } from "../types";
import { UserPreferences } from "../types";

// Sử dụng lại kiểu từ storageService để đồng bộ
export type SyncQueueItem = StorageSyncQueueItem;

export const SYNC_HISTORY_STORAGE_KEY = "commutecast_sync_history_log";
export const MAX_SYNC_HISTORY_COUNT = 100;

/**
 * Retrieves the list of last synchronization events (up to 10 items)
 */
export function getSyncHistory(): SyncHistoryEvent[] {
  try {
    ...
    console.error("[SyncHistory] Failed to parse sync history log:", err);
    return [];
  }
}

/**
 * Records a successful or partial sync event to history log
 */
export function recordSyncHistoryEvent(event: Omit<SyncHistoryEvent, "id" | "timestamp"> & { id?: string; timestamp?: string }): SyncHistoryEvent[] {
  try {
    const current = getSyncHistory();
    const newEvent: SyncHistoryEvent = {
      id: event.id || `sync-event-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      timestamp: event.timestamp || new Date().toISOString(),
      type: event.type,
      status: event.status,
      itemsCount: event.itemsCount,
      details: event.details,
      triggerSource: event.triggerSource || "auto"
    };

    const updated = [newEvent, ...current].slice(0, MAX_SYNC_HISTORY_COUNT);
    if (typeof localStorage !== "undefined") {
      localStorage.setItem(SYNC_HISTORY_STORAGE_KEY, JSON.stringify(updated));
    }

    if (typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent("commutecast-sync-history-updated", { detail: updated }));
    }

    return updated;
  } catch (err) {
    console.error("[SyncHistory] Failed to record sync event:", err);
    return getSyncHistory();
  }
}

/**
 * Clears sync event history
 */
export function clearSyncHistory(): void {
  try {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent("commutecast-sync-history-updated", { detail: [] }));
    }
  } catch (err) {
    console.error("[SyncHistory] Failed to clear history:", err);
  }
}
...
export function isOnline(): boolean {
  if (isOfflineModeEnabled()) {
    return false; // Force offline mode enabled by user
  }
  ...
  try {
    ...
    if (briefingsToSave.size > 0) {
      ...
      for (let i = 0; i < sortedBriefings.length; i += BATCH_SIZE) {
        ...
        if (error) {
          console.error("[Sync] Batch upsert briefings failed:", error);
          if (error.code === '403') {
             console.warn("[Sync] Supabase usage limit reached (403). Queuing for later.");
             return false; // Don't throw, just stop and retry later
          }
        }
      }
    }
    ...
    if (voiceHistoryToSave.size > 0) {
      ...
      for (let i = 0; i < voiceItems.length; i += BATCH_SIZE) {
        ...
        if (error) {
          console.error("[Sync] Batch upsert voice history failed:", error);
    ...
    // Ghi nhận sự kiện đồng bộ hàng đợi thành công vào Lịch sử đồng bộ
    recordSyncHistoryEvent({
      type: "queue_batch",
      status: "success",
      itemsCount: queue.length,
      details: {
        briefingsUploaded: briefingsToSave.size,
        voiceHistorySynced: voiceHistoryToSave.size,
        preferencesSynced: !!latestPreferences
      },
      triggerSource: "queue_debounce"
    });
...
export async function performFullSyncAsync(): Promise<boolean> {
  ...
  try {
    ...
    // Ghi nhận sự kiện toàn diện vào Lịch sử đồng bộ
    recordSyncHistoryEvent({
      type: "full_sync",
      status: "success",
      itemsCount: (cloudBriefings?.length || 0) + localBriefingsMap.size,
      details: {
        briefingsDownloaded: cloudBriefings?.length || 0,
        briefingsUploaded: localBriefingsMap.size,
