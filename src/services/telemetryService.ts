...
import { isLowPowerModeActiveSync } from "../utils/batteryUtils";
...
class TelemetryService {
  ...
  public track(type: TelemetryEvent["type"], payload: any) {
    const isLowPower = isLowPowerModeActiveSync();
    // Reduce telemetry frequency by 80% (effective sampling rate 0.2 max) during Low Power Mode / Low Battery (<20%)
    const effectiveSamplingRate = isLowPower ? Math.min(this.samplingRate, 0.2) : this.samplingRate;
    if (Math.random() > effectiveSamplingRate) return;
    const now = Date.now();
