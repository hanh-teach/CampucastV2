...
import { getUnifiedUserProfile, getPersonalMemory } from "../features/store";

/**
 * Advanced AI-driven Recommendation Engine.
 * Tailors videos to user's interaction history (topics scored in UnifiedUserProfile),
 * AI Personal Memory facts (long-term and short-term topics),
...
export async function scoreAndSortRecommendations(
  ...
  try {
    ...
    const activeMemoryItems = (memory.favoriteTopics || []).filter(item => item.enabled !== false);
    ...
    return videos.map(video => {
      ...
      // 1. Personal AI Memory Boost (from PersonalMemory store)
      activeMemoryItems.forEach(mem => {
        const memTopicLower = mem.topic.toLowerCase();
        if (titleLower.includes(memTopicLower) || descriptionLower.includes(memTopicLower)) {
          // Extra weight if short-term or high interaction count
          const weight = mem.type === "short_term" ? 400 : 250;
          score += weight + (mem.interactedCount || 1) * 20;
        }
      });

      // 2. Common Personalization boost
