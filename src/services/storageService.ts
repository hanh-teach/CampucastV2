import { SavedSummary, VoiceHistoryItem, RSSFeed, SavedReadingItem, ReadHistoryItem } from "../types";
...
const SAVED_READING_KEY = "commute_cast_saved_reading_list";
const READ_HISTORY_KEY = "commute_cast_read_history_list";
...
// ================== SAVED READING LIST (READ LATER) ==================
export function getSavedReadingList(): SavedReadingItem[] {
  try {
    if (typeof localStorage === "undefined") return [];
    const raw = localStorage.getItem(SAVED_READING_KEY);
    return raw ? JSON.parse(raw) : [];
  } catch (err) {
    console.error("Failed to load saved reading list:", err);
    return [];
  }
}

export function saveReadingItem(item: Omit<SavedReadingItem, "id" | "savedAt"> & { id?: string; savedAt?: string }): SavedReadingItem[] {
  try {
    const current = getSavedReadingList();
    const itemId = item.id || item.url || item.title;
    const exists = current.some((x) => x.id === itemId || x.url === item.url);
    if (!exists) {
      ...
      localStorage.setItem(SAVED_READING_KEY, JSON.stringify(updated));
      return updated;
    }
    return current;
  } catch (err) {
...
export function removeReadingItem(urlOrId: string): SavedReadingItem[] {
  try {
    const current = getSavedReadingList();
    const updated = current.filter((x) => x.id !== urlOrId && x.url !== urlOrId);
    localStorage.setItem(SAVED_READING_KEY, JSON.stringify(updated));
    return updated;
  } catch (err) {
    console.error("Failed to remove reading item:", err);
    return getSavedReadingList();
  }
}

export function isArticleInSavedReading(urlOrId: string): boolean {
  if (!urlOrId) return false;
  const current = getSavedReadingList();
  return current.some((x) => x.id === urlOrId || x.url === urlOrId);
}

// ================== READ HISTORY (AUTOMATIC LOGGING) ==================
const MAX_READ_HISTORY_LIMIT = 150;

export function getReadHistoryList(): ReadHistoryItem[] {
  try {
    if (typeof localStorage === "undefined") return [];
    const raw = localStorage.getItem(READ_HISTORY_KEY);
    return raw ? JSON.parse(raw) : [];
  } catch (err) {
    console.error("Failed to load read history list:", err);
    return [];
  }
}

export function recordReadHistoryItem(
  item: Omit<ReadHistoryItem, "id" | "readAt"> & { id?: string; readAt?: string }
): ReadHistoryItem[] {
  try {
    if (!item.title) return getReadHistoryList();
    const current = getReadHistoryList();
    const itemId = item.id || item.url || item.title;

    // Filter out existing record if re-reading to move it to the top with fresh timestamp
    const filtered = current.filter((x) => x.id !== itemId && x.url !== item.url && x.title !== item.title);
    ...
    const newItem: ReadHistoryItem = {
      ...
      feedTitle: item.feedTitle,
      category: item.category,
      contentSnippet: item.contentSnippet ? item.contentSnippet.slice(0, 300) : undefined,
      pubDate: item.pubDate,
      feedType: item.feedType,
    };

    const updated = [newItem, ...filtered].slice(0, MAX_READ_HISTORY_LIMIT);
    localStorage.setItem(READ_HISTORY_KEY, JSON.stringify(updated));
    return updated;
  } catch (err) {
    console.error("Failed to record read history item:", err);
    return getReadHistoryList();
  }
}

export function removeReadHistoryItem(idOrUrl: string): ReadHistoryItem[] {
  try {
    const current = getReadHistoryList();
    const updated = current.filter((x) => x.id !== idOrUrl && x.url !== idOrUrl && x.title !== idOrUrl);
    localStorage.setItem(READ_HISTORY_KEY, JSON.stringify(updated));
    return updated;
  } catch (err) {
    console.error("Failed to remove read history item:", err);
    return getReadHistoryList();
  }
}

export function clearReadHistoryList(): ReadHistoryItem[] {
  try {
    ...
    console.error("Failed to clear read history list:", err);
    return [];
  }
}