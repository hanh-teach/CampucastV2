...
const MAX_STORAGE_AGE_MS = 7 * 24 * 60 * 60 * 1000; // 7 days stale eviction threshold
 ...
 * Supports instant memory lookup, safe JSON parsing, and localStorage persistence for offline survival.
 */
  ...
  get(key: string): FeedResponse | null {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (stored) {
          const entry = safeJsonParse<CacheEntry | null>(stored, null);
          if (entry && entry.videos) {
