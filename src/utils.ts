...
/**
 * Strips WAV header if present, returning pure PCM ArrayBuffer.
 */
export function stripWavHeaderInBrowser(ab: ArrayBuffer): ArrayBuffer {
  if (ab.byteLength >= 44) {
    const view = new DataView(ab);
    if (view.getUint8(0) === 0x52 && view.getUint8(1) === 0x49 && view.getUint8(2) === 0x46 && view.getUint8(3) === 0x46) {
      let offset = 12;
      while (offset + 8 <= ab.byteLength) {
        const char0 = String.fromCharCode(view.getUint8(offset));
        const char1 = String.fromCharCode(view.getUint8(offset + 1));
        const char2 = String.fromCharCode(view.getUint8(offset + 2));
        const char3 = String.fromCharCode(view.getUint8(offset + 3));
        const subchunkId = char0 + char1 + char2 + char3;
        const subchunkSize = view.getUint32(offset + 4, true);
        if (subchunkId === 'data') {
          return ab.slice(offset + 8, Math.min(ab.byteLength, offset + 8 + subchunkSize));
        }
...
/**
 * Convert Float32Array audio samples [-1.0, 1.0] to 16-bit PCM little-endian ArrayBuffer.
 */
export function float32ToInt16Pcm(floatArray: Float32Array): ArrayBuffer {
  const pcmBuffer = new ArrayBuffer(floatArray.length * 2);
...
/**
 * Applies a Hann window fade-in and fade-out to Float32Array samples to eliminate clicks and pops.
 */
export function applyHannMicroFade(floatArray: Float32Array, fadeSamples: number = 240): void {
  const actualFade = Math.min(fadeSamples, Math.floor(floatArray.length / 2));
  ...
  // Fade-out
  for (let i = 0; i < actualFade; i++) {
    const idx = floatArray.length - actualFade + i;
    const window = 0.5 * (1 - Math.cos((Math.PI * (actualFade - 1 - i)) / actualFade));
    floatArray[idx] *= window;
  }
}

/**
 * Checks if an ArrayBuffer starts with ID3 or MPEG Audio Sync frame.
 */
export function isMp3ArrayBuffer(ab: ArrayBuffer): boolean {
  ...
  // MPEG Frame Sync
  if (b0 === 0xFF && (b1 & 0xE0) === 0xE0 && (b1 & 0x18) !== 0x08) return true;
  return false;
}
