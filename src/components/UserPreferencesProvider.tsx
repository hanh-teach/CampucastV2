...
const DEFAULT_PREFERENCES: BroadcastConfiguration = {
  ...
  scheduledBriefingTime: "07:30",
  favoriteTopics: ["Thời sự", "Công nghệ", "Giao thông"],
  trafficAlertsEnabled: true,
  autoAudioAlertsEnabled: false,
  isOfflineMode: false,
  isLowPowerModeEnabled: false,
  autoLowPowerThreshold: 20
};
  ...
  const [preferences, setPreferences] = useState<BroadcastConfiguration>(() => {
    try {
      ...
      if (saved) {
        ...
        const scheduledBriefingEnabled = parsed.scheduledBriefingEnabled !== undefined ? parsed.scheduledBriefingEnabled : DEFAULT_PREFERENCES.scheduledBriefingEnabled;
        const scheduledBriefingTime = parsed.scheduledBriefingTime || DEFAULT_PREFERENCES.scheduledBriefingTime;
        const favoriteTopics = parsed.favoriteTopics || DEFAULT_PREFERENCES.favoriteTopics;
        const trafficAlertsEnabled = parsed.trafficAlertsEnabled !== undefined ? parsed.trafficAlertsEnabled : DEFAULT_PREFERENCES.trafficAlertsEnabled;
        const autoAudioAlertsEnabled = parsed.autoAudioAlertsEnabled !== undefined ? parsed.autoAudioAlertsEnabled : DEFAULT_PREFERENCES.autoAudioAlertsEnabled;
        const isOfflineMode = parsed.isOfflineMode !== undefined ? parsed.isOfflineMode : DEFAULT_PREFERENCES.isOfflineMode;
        const isLowPowerModeEnabled = parsed.isLowPowerModeEnabled !== undefined ? parsed.isLowPowerModeEnabled : DEFAULT_PREFERENCES.isLowPowerModeEnabled;
        const autoLowPowerThreshold = parsed.autoLowPowerThreshold !== undefined ? parsed.autoLowPowerThreshold : DEFAULT_PREFERENCES.autoLowPowerThreshold;
  ...
  useEffect(() => {
    try {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("commutecast-user-preferences-updated", { detail: preferences }));
      }
