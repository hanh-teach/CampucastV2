...
import { getStorageEstimate, getAllBriefings } from "../../services/storageService";
const BuildHealthDashboard = lazy(() => import("../BuildHealthDashboard"));
...
export default function SettingsTabView({
  ...
  const handleExportBriefingData = async () => {
    try {
      setIsExportingData(true);
      setExportSuccessMsg(null);
      
      const briefings = await getAllBriefings(false);
      const exportPayload = {
        appName: "CommuteCast",
        version: "7.105.3",
        exportedAt: new Date().toISOString(),
        totalRecords: briefings.length,
        briefings: briefings
      };
      ...
      const filename = `commutecast_briefings_backup_${dateStr}.json`;
      ...
      const successText = uiLanguage === "vi"
        ? `Đã xuất thành công ${briefings.length} bản ghi lịch sử bản tin ra tập tin JSON!`
        : `Successfully exported ${briefings.length} briefing records to JSON file!`;
      
      setExportSuccessMsg(successText);
      setTimeout(() => setExportSuccessMsg(null), 4000);
    } catch (err) {
      console.error("[Settings] Failed to export briefing history:", err);
      alert(uiLanguage === "vi" ? "Lỗi khi xuất dữ liệu tổng hợp. Vui lòng thử lại." : "Failed to export briefing history data. Please try again.");
    } finally {
        ...
        mainPanel={
          <div className="max-w-3xl mx-auto space-y-12">
            ...
            <div className="space-y-10">
              {activeCategory === "general" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                  <SettingsCenter
                    ...
                    onClearAllCache={() => onClearAllCache({ clearCloud: false })}
                    onTestTrafficAlert={onTestTrafficAlert}
                  />
                ...
                <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <Card className="p-8 border-border-subtle space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start sm:items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-black uppercase tracking-tight text-text-main">
                              {uiLanguage === "vi" ? "Chế độ Ngoại tuyến (Offline Mode)" : "Offline Mode"}
                            </h4>
                            {preferences.isOfflineMode && (
                              <Badge className="bg-amber-500/20 text-amber-500 border-amber-500/30 text-[9px] font-mono uppercase px-2 py-0.5">
                          ...
                          <p className="text-xs text-text-muted mt-1 leading-relaxed">
                            {uiLanguage === "vi"
                              ? "Khóa tải dữ liệu qua mạng. Tất cả các tác vụ cập nhật RSS và bản tin sẽ chỉ đọc dữ liệu từ bộ nhớ đệm local."
                              : "Explicitly restricts data fetching to cached local storage. Ideal for tunnels or low-connectivity commutes."}
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => updatePreference("isOfflineMode", !preferences.isOfflineMode)}
                        className={cn(
                          "relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none self-start sm:self-auto",
                      ...
                      >
                        <span
                          className={cn(
                            ...
                            preferences.isOfflineMode ? "translate-x-5" : "translate-x-0"
                          )}
                        />
                      </button>
                    </div>
                  ...
                  <Card className="p-8 border-border-subtle space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {preferences.isLowPowerModeEnabled && (
                              <Badge className="bg-emerald-500/20 text-emerald-500 border-emerald-500/30 text-[9px] font-mono uppercase px-2 py-0.5">
                                {uiLanguage === "vi" ? "Đã bật" : "ENABLED"}
                              </Badge>
                            )}
                            <Badge className="bg-amber-500/15 text-amber-500 border-amber-500/30 text-[9px] font-mono uppercase px-2 py-0.5">
                              {uiLanguage === "vi" ? "Tự động khi Pin < 20%" : "Auto < 20% Battery"}
                            </Badge>
                          </div>
                          <p className="text-xs text-text-muted mt-1 leading-relaxed">
                            {uiLanguage === "vi"
                              ? "Giảm 80% tần suất gửi telemetry và tạm dừng tự động kiểm tra bài viết RSS ngầm khi mức pin thiết bị rơi xuống dưới 20%."
                              : "Reduces telemetry frequency by 80% and pauses background RSS polling when device battery falls below 20%."}
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => updatePreference("isLowPowerModeEnabled", !preferences.isLowPowerModeEnabled)}
                        className={cn(
                          "relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none self-start sm:self-auto",
                      ...
                      >
                        <span
                          className={cn(
                            "pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out",
                            preferences.isLowPowerModeEnabled ? "translate-x-5" : "translate-x-0"
                          )}
                        />
                      </button>
                    </div>
                  ...
                  {/* Build Health & Chunk Telemetry Dashboard */}
                  <Suspense fallback={<div className="p-8 text-center text-text-dim text-xs font-mono animate-pulse">Loading Build Health Telemetry...</div>}>
                    <BuildHealthDashboard uiLanguage={uiLanguage} />
                  </Suspense>
                </div>
              )}
              ...
              {activeCategory === "sync" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                  {/* Dedicated Offline Mode Toggle Switch */}
                  <Card className={cn(
                    "p-6 sm:p-8 border transition-all space-y-4",
                    preferences.isOfflineMode 
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <div className={cn(
                          "p-3 rounded-2xl shrink-0 transition-colors mt-0.5 sm:mt-0",
                          preferences.isOfflineMode
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {preferences.isOfflineMode ? (
                              <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-600 dark:text-amber-400 uppercase tracking-wider">
                                {uiLanguage === "vi" ? "CƯỠNG ÉP NGOẠI TUYẾN" : "FORCE OFFLINE"}
                              </span>
                            ) : (
                              <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
                                {uiLanguage === "vi" ? "Đồng bộ Đám mây Mở" : "Cloud Sync Allowed"}
                              </span>
                            )}
                          </div>
                          <p className="text-xs text-text-muted leading-relaxed">
                            {uiLanguage === "vi"
                              ? "Bắt buộc ứng dụng làm việc độc quyền với dữ liệu lưu tạm cục bộ, vô hiệu hóa tất cả các yêu cầu đồng bộ đám mây cho đến khi mở lại."
                              : "Forces the app to work exclusively with cached local data, disabling all cloud sync requests until re-enabled."}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
                        <span className="text-xs font-mono font-bold text-text-muted uppercase tracking-wider">
                          {preferences.isOfflineMode ? (uiLanguage === "vi" ? "BẬT" : "ON") : (uiLanguage === "vi" ? "TẮT" : "OFF")}
                        </span>
                        <button
                          type="button"
                          role="switch"
                          aria-checked={Boolean(preferences.isOfflineMode)}
                          onClick={() => updatePreference("isOfflineMode", !preferences.isOfflineMode)}
                          className={cn(
                            "relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2",
                            preferences.isOfflineMode ? "bg-amber-500" : "bg-neutral-300 dark:bg-neutral-700"
                          )}
                        >
                          <span
                            className={cn(
                      ...
                      <div className="space-y-1 text-left">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={cn(
                            ...
                            preferences.isOfflineMode && "bg-amber-500",
                            !preferences.isOfflineMode && syncStatus === "synced" && "bg-green-500",
                            !preferences.isOfflineMode && syncStatus === "syncing" && "bg-blue-500",
                            !preferences.isOfflineMode && syncStatus === "offline" && "bg-gray-500",
                            !preferences.isOfflineMode && syncStatus === "error" && "bg-red-500",
                            !preferences.isOfflineMode && syncStatus === "unauthenticated" && "bg-amber-500"
                          )} />
                          <h5 className="text-sm font-bold uppercase tracking-tight">
                            {preferences.isOfflineMode
                              ? (uiLanguage === "vi" ? "Chế độ Ngoại tuyến (Cưỡng ép Cục bộ)" : "Offline Mode (Forced Local)")
                              : {
                    ...
                    {!user ? (
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <Button 
                          ...
                          onClick={triggerSync}
                          disabled={syncStatus === "syncing" || !isOnline || preferences.isOfflineMode}
                        >
                          {preferences.isOfflineMode
                            ? (uiLanguage === "vi" ? "Khóa (Đang bật Chế độ Ngoại tuyến)" : "Disabled (Offline Mode Active)")
                            : syncStatus === "syncing" 
                  ...
                  <Card className="p-6 sm:p-8 border-border-subtle space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1 text-left">
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-black uppercase tracking-tight text-text-primary">
                              {uiLanguage === "vi" ? "Xuất Dữ liệu Cục bộ (JSON Export)" : "Data Export (JSON Backup)"}
                            </h4>
                            <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-surface-subtle border border-border-subtle text-text-muted">
                              JSON
                            </span>
                          </div>
                          <p className="text-xs text-text-muted leading-relaxed">
                            {uiLanguage === "vi"
                              ? "Tải về toàn bộ lịch sử bản tin tin tức và thông tin tổng hợp trên thiết bị dưới dạng tệp tin JSON để sao lưu thủ công."
                              : "Download all local briefing history records saved on this device as a JSON file for manual backup."}
                          </p>
                        </div>
                      </div>
                      ...
                      <Button
                        ...
                        onClick={handleExportBriefingData}
                        className="h-11 px-5 border-brand-accent/30 text-brand-accent hover:bg-brand-accent/10 transition-all font-black uppercase tracking-wider text-[11px] shrink-0 self-start sm:self-center"
                      >
                        {isExportingData ? (
                          <>
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                            {uiLanguage === "vi" ? "Đang xuất..." : "Exporting..."}
                          </>
                        ) : (
                          <>
                            <Download className="w-4 h-4 mr-2" />
                            {uiLanguage === "vi" ? "Xuất Lịch sử Bản tin" : "Export Briefing Data"}
                          </>
                        )}
                      </Button>
                    </div>
                    ...
                    {exportSuccessMsg && (
                      <div className="p-3.5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex items-center gap-3 text-xs text-emerald-600 dark:text-emerald-400 font-medium animate-in fade-in slide-in-from-top-1 text-left">
                  ...
                  <SyncConflicts uiLanguage={uiLanguage} />

                  {/* Sync History Events Log */}
                  <SyncHistory uiLanguage={uiLanguage} />
                </div>
              )}
