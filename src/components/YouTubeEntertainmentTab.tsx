...
export const YouTubeEntertainmentTab = React.memo(function YouTubeEntertainmentTab({
  ...
  // Embedded Voice Search inside YouTube tab
  const { isListening, interimTranscript, startListening, stopListening } = useSpeechRecognition({
    ...
    onTranscript: (text) => {
      if (text && text.trim()) {
        ...
        setSearchStatus(uiLanguage === "vi" ? `Đang tìm: "${text}"...` : `Searching for: "${text}"...`);
        setTimeout(() => setSearchStatus(""), 2500);
      }
    }
  });

  // Handle Voice Search from parent / Driving HUD
  useEffect(() => {
    if (voiceSearchQuery) {
      setSearchStatus(uiLanguage === "vi" ? `Đang tìm: "${voiceSearchQuery}"...` : `Searching for: "${voiceSearchQuery}"...`);
      setSearchInput(voiceSearchQuery);
  ...
  // Synchronize incoming Voice Command Action from Driving HUD
  useEffect(() => {
    if (!voiceCommandAction) return;
    switch (voiceCommandAction.type) {
      case "PLAY":
  ...
  }, [voiceCommandAction, isPlaying, handlePlayPause, handleNextVideo, handlePrevVideo]);
  ...
  return (
    <>
      ...
      {/* CỘT PHẢI - Thanh danh mục dọc & Tìm kiếm */}
      <div className="flex-[1] max-w-[210px] flex flex-col justify-start gap-2.5 pt-1">
        {!isAuthenticated && (
          <button onClick={connectYouTube} className="w-full text-center py-2.5 bg-red-600 rounded-lg text-white font-bold text-xs mb-1 hover:bg-red-700 active:scale-95 transition-all">
            {uiLanguage === "vi" ? "KẾT NỐI YOUTUBE" : "CONNECT YOUTUBE"}
          </button>
        )}

        {/* Interactive Search Bar & Voice Mic Button */}
        <form onSubmit={handleManualSearchSubmit} className="relative w-full flex items-center gap-1.5">
          <div className="relative flex-1 min-w-0">
            <input
              type="text"
              value={searchInput}
              onChange={(e) => setSearchInput(e.target.value)}
              placeholder={uiLanguage === "vi" ? "Tìm YouTube..." : "Search YouTube..."}
              className="w-full bg-zinc-900 border border-white/10 rounded-xl py-2 pl-7 pr-6 text-xs text-white placeholder:text-zinc-500 focus:outline-none focus:border-red-500/50"
            />
            ...
            {searchInput && (
              <button
                ...
                onClick={() => {
                  setSearchInput("");
                  if (category === "Search Results") setCategory("Trending");
                }}
          ...
          <button
            ...
            className={cn(
              "p-2 rounded-xl border transition-all active:scale-90 flex items-center justify-center shrink-0",
              isListening
                ? "bg-red-600 text-white border-red-400 animate-pulse"
                : "bg-white/5 border-white/10 text-zinc-300 hover:bg-white/10 hover:text-white"
            )}
            title={uiLanguage === "vi" ? "Tìm bằng giọng nói" : "Voice search"}
          >
        ...
        {isListening && (
          <div className="p-2 bg-red-950/60 border border-red-500/30 rounded-xl text-[10px] text-red-300 flex items-center gap-1.5 animate-pulse">
            <Mic className="w-3 h-3 text-red-400 shrink-0" />
            <span className="truncate">{interimTranscript || (uiLanguage === "vi" ? "Nói từ khóa..." : "Speak search term...")}</span>
          </div>
        )}

        {category === "Search Results" && activeSearchQuery && (
          <div className="flex items-center justify-between p-2 bg-red-600/20 border border-red-500/30 rounded-xl text-[10px] text-red-300">
            <span className="truncate font-bold">🔍 {activeSearchQuery}</span>
            <button
              onClick={() => {
                setActiveSearchQuery("");
                setSearchInput("");
        ...
        <div className="w-full flex items-center justify-between pb-1.5 border-b border-white/10 my-0.5 px-1">
