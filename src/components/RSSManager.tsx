...
import { saveRSSFeed, getRSSFeeds, deleteRSSFeed, getSavedReadingList } from "../services/storageService";
import { fetchRSSArticles, formatArticlesForPrompt, exportToOPML, parseOPML, analyzeArticlesSentimentWithAI } from "../services/rssService";
import { checkForNewRSSArticles, getRefreshIntervalMinutes, setRefreshIntervalMinutes } from "../services/schedulerService";
import RSSFeedList from "./RSSFeedList";
import { FeedSparkline } from "./FeedSparkline";
...
export default function RSSManager({
  ...
  const handleAnalyzeArticlesSentiment = async () => {
    if (activeArticles.length === 0 || isAnalyzingSentiment) return;
    setIsAnalyzingSentiment(true);
    setErrorMsg("");
    setSuccessMsg("");
    try {
      const updatedArticles = await analyzeArticlesSentimentWithAI(activeArticles, getApiUrl, uiLanguage);
      setActiveArticles(updatedArticles);
      setSuccessMsg(
        uiLanguage === "vi"
          ? `Đã phân tích cảm xúc Gemini AI cho ${updatedArticles.length} bài viết.`
          : `Analyzed sentiment for ${updatedArticles.length} articles with Gemini AI.`
      );
    } catch (err: any) {
      setErrorMsg(
        uiLanguage === "vi"
          ? `Lỗi khi phân tích cảm xúc: ${err.message || String(err)}`
          : `Sentiment analysis error: ${err.message || String(err)}`
      );
    } finally {
  ...
  const [briefCategory, setBriefCategory] = useState<string>("All");
  ...
  const [refreshInterval, setRefreshInterval] = useState<number>(() => getRefreshIntervalMinutes());
  const [rssNotification, setRssNotification] = useState<{ message: string; type: "error" | "warning" | "success" | "confirm" | "info"; action?: () => void } | null>(null);
  ...
  const handleRefreshIntervalChange = (minutes: number) => {
    ...
    setRssNotification({
      ...
      message: uiLanguage === "vi"
        ? `Đã cài đặt chu kỳ tự động kiểm tra RSS nền: ${minutes} phút/lần!`
        : `Background RSS check frequency set to every ${minutes} minutes!`
    });
  };
  ...
  const t = {
    ...
    btnRefreshFeeds: uiLanguage === "vi" ? "Làm mới Nguồn RSS" : "Refresh RSS Feeds"
  };
  ...
  const handleToggleStarFeed = async (feedToToggle: RSSFeed) => {
    try {
      const updated: RSSFeed = {
        ...feedToToggle,
        isStarred: !feedToToggle.isStarred
      };
      await saveRSSFeed(updated);
      setFeeds(prev => prev.map(f => f.id === feedToToggle.id ? updated : f));
    } catch (err) {
      console.error("Failed to toggle feed star priority:", err);
    }
  };

  const handleAutoCategorizeFeeds = async (onlyUnassigned: boolean = false) => {
    if (feeds.length === 0) return;

    // Filter target feeds if requested, or process all feeds
    const targetFeeds = onlyUnassigned
      ? feeds.filter(f => !f.category || f.category === "Khác" || f.category === "General" || f.category === "Tin tức" || f.category.trim() === "")
      : feeds;
    ...
    if (targetFeeds.length === 0) {
      setRssNotification({
        type: "warning",
        message: uiLanguage === "vi"
          ? "Tất cả các nguồn tin hiện tại đã được gán danh mục cụ thể!"
          : "All RSS feeds already have specific categories assigned!"
      });
      return;
    }
    ...
    try {
      const response = await fetch(getApiUrl("/api/auto-categorize-feeds"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          feeds: targetFeeds.map(f => ({
            id: f.id,
            title: f.title,
            url: f.url,
            category: f.category
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || (uiLanguage === "vi" ? "Lỗi phân loại AI từ hệ thống." : "Failed to auto-categorize feeds with AI."));
      }

      const data = await response.json();
      const suggestions: Array<{ id: string; suggestedCategory: string; reason?: string }> = data.categorized || [];

      if (suggestions.length === 0) {
        throw new Error(uiLanguage === "vi" ? "AI không tạo được danh mục đề xuất." : "AI produced no category suggestions.");
      }

      let updatedCount = 0;
      const updatedFeedsList: RSSFeed[] = [];

      for (const feed of feeds) {
        const match = suggestions.find(s => s.id === feed.id);
        if (match && match.suggestedCategory) {
          updatedCount++;
          const updatedFeed = { ...feed, category: match.suggestedCategory };
          await saveRSSFeed(updatedFeed);
          updatedFeedsList.push(updatedFeed);
        } else {
          updatedFeedsList.push(feed);
        }
      }

      setFeeds(updatedFeedsList);

      // Update categories in activeArticles if any
      if (activeArticles.length > 0) {
        const updatedArticlesList = activeArticles.map(art => {
          const parentFeed = updatedFeedsList.find(f => f.id === art.feedId);
          if (parentFeed) {
            return { ...art, feedCategory: parentFeed.category, category: parentFeed.category };
          }
          return art;
        });
        setActiveArticles(updatedArticlesList);
      }

      setRssNotification({
        type: "success",
        message: uiLanguage === "vi"
          ? `Đã phân loại AI thành công cho ${updatedCount} nguồn tin!`
          : `Successfully auto-categorized ${updatedCount} RSS feeds with AI!`
      });
    } catch (err: any) {
      console.error("Auto categorize error:", err);
      setRssNotification({
        type: "error",
        message: err.message || (uiLanguage === "vi" ? "Lỗi khi tự động phân loại bằng AI." : "Error auto-categorizing feeds with AI.")
      });
    } finally {
  ...
  // Refresh RSS Feeds (triggers schedulerService logic and updates UI instantly)
  const handleRefreshFeeds = async () => {
    ...
    setSuccessMsg("");
    try {
      await checkForNewRSSArticles(getApiUrl, true);
      const articles = await fetchRSSArticles(feeds, getApiUrl, true);
      setActiveArticles(articles);
      setLastSyncedAt(new Date().toLocaleTimeString());
      setSuccessMsg(
        uiLanguage === "vi"
          ? `Làm mới thành công! Đã tải ${articles.length} bài viết mới nhất.`
          : `Successfully refreshed feeds! Loaded ${articles.length} latest articles.`
      );
    } catch (err: any) {
      setErrorMsg(err.message || "Failed to refresh RSS feeds.");
    } finally {
      setIsFetchingArticles(false);
    }
  };
  ...
  const triggerBriefingGenerationBrief = (bypassConfirm = false) => {
    ...
    // Filter by category and date type
    let filtered = activeArticles.filter(art => {
      const matchCat = briefCategory === "All" || 
        (art.feedCategory && art.feedCategory.toLowerCase() === briefCategory.toLowerCase()) ||
        (art.category && art.category.toLowerCase() === briefCategory.toLowerCase());
      const matchDate = isArticleInDateRange(art.pubDate || "", briefDateType, briefStartDate, briefEndDate);
    ...
    if (filtered.length === 0) {
      setRssNotification({
        type: "warning",
        message: uiLanguage === "vi"
          ? `Không tìm thấy bài viết phù hợp${briefCategory !== "All" ? ` cho chủ đề "${briefCategory}"` : ""}. Hãy thử: Chọn Tất cả chủ đề, Mở rộng khoảng ngày hoặc Tải lại nguồn tin.`
          : `No matching articles found${briefCategory !== "All" ? ` for category "${briefCategory}"` : ""}. Try: selecting All Categories, expanding date range, or refreshing feeds.`
      });
      return;
    }
  ...
  return (
    <div className="space-y-6" id="rss-composite-module">
      ...
      <Card className="flex flex-col gap-5 p-6 transition-colors" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
        {/* Title & Desc */}
        <div className="flex items-center gap-2.5">
          <div className="p-2.5 rounded-app-xl border flex items-center justify-center"
               style={{ backgroundColor: `${colors.interactive}1a`, color: colors.interactive, borderColor: `${colors.interactive}33`, minWidth: "44px", minHeight: "44px" }}>
            <Rss className="w-5 h-5" />
          </div>
          <div className="text-left">
            <h3 className="text-sm font-extrabold flex items-center gap-1.5" style={{ color: colors.textPrimary }}>
              <span>{t.managerTitle}</span>
              <Badge variant="secondary" className="text-[9px] font-extrabold px-1.5 py-0.5 uppercase">
                {uiLanguage === "vi" ? "Đa Kênh" : "Multi-channel"}
              </Badge>
            </h3>
            <p className="text-[11px] mt-1 font-medium leading-relaxed" style={{ color: colors.textSecondary }}>
              {t.managerDesc}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSaveFeed} className="space-y-4 p-5 border rounded-app-2xl transition-colors"
              style={{ backgroundColor: colors.surfaceOverlay, borderColor: colors.border }}>
          <div className="text-left">
            <Badge variant="outline" className="text-[10px] font-extrabold uppercase tracking-wide">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <input
                ...
                required
                className="text-xs px-4 py-3 rounded-app-xl focus:outline-none transition-all font-medium border"
                style={{ backgroundColor: colors.surface, borderColor: colors.border, color: colors.textPrimary, minHeight: "44px" }}
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              <input
                type="text"
                value={newIncludeKeywords}
                onChange={(e) => setNewIncludeKeywords(e.target.value)}
                placeholder={uiLanguage === "vi" ? "Chỉ muốn nghe về (phân cách bằng dấu phẩy)..." : "Include Keywords (comma separated)..."}
                className="text-xs px-4 py-3 rounded-app-xl focus:outline-none transition-all font-medium border"
                style={{ backgroundColor: colors.surface, borderColor: colors.border, color: colors.textPrimary, minHeight: "44px" }}
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              <input
                type="text"
                value={newExcludeKeywords}
                onChange={(e) => setNewExcludeKeywords(e.target.value)}
                placeholder={uiLanguage === "vi" ? "Không muốn nghe về (phân cách bằng dấu phẩy)..." : "Exclude Keywords (comma separated)..."}
                className="text-xs px-4 py-3 rounded-app-xl focus:outline-none transition-all font-medium border"
                style={{ backgroundColor: colors.surface, borderColor: colors.border, color: colors.textPrimary, minHeight: "44px" }}
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="text-[11px] px-3 py-2 rounded-app-lg focus:outline-none font-semibold cursor-pointer border transition-colors"
                  style={{ backgroundColor: colors.surface, borderColor: colors.border, color: colors.textPrimary, minHeight: "40px" }}
                >
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-1" style={{ color: colors.textMuted }}>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {isTesting ? (
                  ...
                  <>
                    {editingFeedId ? <Check className="w-3.5 h-3.5 mr-1.5" /> : <Plus className="w-3.5 h-3.5 mr-1.5" />}
        ...
        <div className="space-y-4 border-t pt-5 text-left" style={{ borderColor: colors.border }}>
          {(() => {
            const filteredFeeds = feeds
              .filter((feed) => {
                const matchCat = feedCategoryFilter === "All" || (feed.category && feed.category.toLowerCase() === feedCategoryFilter.toLowerCase());
                if (!matchCat) return false;
                if (!feedSearchTerm.trim()) return true;
                ...
                return (
                  ...
                  (feed.category && feed.category.toLowerCase().includes(term))
                );
              })
              .sort((a, b) => {
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider flex items-center gap-1.5" style={{ color: colors.textPrimary }}>
                    ...
                    <span>{t.rssListHeader} ({filteredFeeds.length}{feedSearchTerm.trim() ? `/${feeds.length}` : ""})</span>
                  </h4>
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    {feeds.length > 0 && (
                      <>
                        <button
                          type="button"
                          onClick={() => handleAutoCategorizeFeeds(false)}
                          disabled={isAutoCategorizing || isFetchingArticles}
                          className="px-3.5 py-2 text-[10px] font-bold border rounded-app-lg flex items-center gap-2 transition-all cursor-pointer hover:bg-opacity-80 active:scale-95 disabled:opacity-50"
                          style={{ backgroundColor: `${colors.interactive}15`, borderColor: `${colors.interactive}40`, color: colors.interactive, minHeight: "36px" }}
                          title={uiLanguage === "vi" ? "Dùng AI Gemini phân tích tiêu đề & URL để tự động gán danh mục chuẩn xác cho các nguồn tin" : "Auto analyze titles & URLs with Gemini AI to assign accurate categories for RSS feeds"}
                        >
                          <Sparkles className={cn("w-3.5 h-3.5", isAutoCategorizing && "animate-spin")} />
                          <span>
                            {isAutoCategorizing 
                              ? (uiLanguage === "vi" ? "Đang phân loại..." : "Categorizing...") 
                              : (uiLanguage === "vi" ? "Tự động phân loại AI" : "AI Auto-Categorize")}
                          </span>
                        </button>
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 text-[10px] font-bold border rounded-app-lg flex items-center gap-2 transition-all cursor-pointer hover:bg-opacity-80 active:scale-95 disabled:opacity-50"
                          style={{ backgroundColor: `${colors.interactive}1a`, borderColor: `${colors.interactive}33`, color: colors.interactive, minHeight: "36px" }}
                          title={uiLanguage === "vi" ? "Làm mới nguồn RSS thủ công & cập nhật ngay lập tức" : "Manually refresh RSS feeds and update UI instantly"}
                        >
                          <RefreshCw className={cn("w-3.5 h-3.5", isFetchingArticles && "animate-spin")} />
                          <span>{t.btnRefreshFeeds}</span>
                        </button>
                ...
                {/* Search, Category filter, and Refresh Frequency inputs for feeds */}
                {feeds.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                    <div className="sm:col-span-2 relative">
                      <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: colors.textMuted }} />
                      <input
                        type="text"
                        value={feedSearchTerm}
                        onChange={(e) => setFeedSearchTerm(e.target.value)}
                        placeholder={uiLanguage === "vi" ? "Lọc nguồn tin theo tên channel hoặc URL..." : "Filter feeds by title or URL..."}
                        className="w-full pl-9 pr-8 py-2 text-xs rounded-app-lg border transition-all focus:outline-none focus:ring-1"
                        style={{
                    ...
                    <div>
                      <select
                        ...
                        className="w-full px-3 py-2 text-xs font-bold rounded-app-lg border transition-all focus:outline-none focus:ring-1 cursor-pointer"
                        style={{
                          backgroundColor: colors.surfaceOverlay,
                          borderColor: colors.border,
                          color: colors.textPrimary,
                      ...
                      >
                        <option value="All">{uiLanguage === "vi" ? "Tất cả danh mục" : "All Categories"}</option>
                        {["Thời sự", "Công nghệ", "Kinh tế", "Giáo dục", "Sức khỏe", "Thể thao", "Giải trí", "Khác"].map(cat => (
                    ...
                    <div>
                      <select
                        ...
                        className="w-full px-3 py-2 text-xs font-bold rounded-app-lg border transition-all focus:outline-none focus:ring-1 cursor-pointer flex items-center gap-1"
                        style={{
                          backgroundColor: `${colors.interactive}10`,
                          borderColor: `${colors.interactive}40`,
                          color: colors.interactive,
                        }}
                        title={uiLanguage === "vi" ? "Tần suất trình kiểm tra tự động quét tin mới từ nguồn RSS" : "Background scheduler check frequency for new RSS articles"}
                      >
                        <option value={15}>{uiLanguage === "vi" ? "⏱ Tần suất: 15 phút" : "⏱ Refresh: 15 mins"}</option>
                        <option value={30}>{uiLanguage === "vi" ? "⏱ Tần suất: 30 phút (Mặc định)" : "⏱ Refresh: 30 mins (Default)"}</option>
                        <option value={60}>{uiLanguage === "vi" ? "⏱ Tần suất: 60 phút (1h)" : "⏱ Refresh: 60 mins (1h)"}</option>
                        <option value={120}>{uiLanguage === "vi" ? "⏱ Tần suất: 120 phút (2h)" : "⏱ Refresh: 120 mins (2h)"}</option>
                        <option value={360}>{uiLanguage === "vi" ? "⏱ Tần suất: 360 phút (6h)" : "⏱ Refresh: 360 mins (6h)"}</option>
                      </select>
                    </div>
                  </div>
                )}
                ...
                {feeds.length === 0 ? (
                  ...
                  <p className="text-xs italic text-center py-8 border border-dashed rounded-app-xl transition-colors"
                     style={{ backgroundColor: colors.surfaceOverlay, borderColor: colors.border, color: colors.textMuted }}>
                    {uiLanguage === "vi"
                      ? `Không tìm thấy nguồn RSS nào khớp với "${feedSearchTerm}".`
                      : `No RSS feeds found matching "${feedSearchTerm}".`}
                  </p>
                ) : (
                  <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                    {filteredFeeds.map((feed) => {
                      const fetchCount = feed.fetchCount || 0;
                      const successCount = feed.successCount || 0;
                      const successRate = fetchCount > 0 ? Math.round((successCount / fetchCount) * 100) : 100;
                      const rawHealth = feed.healthStatus;

                      let healthStatusType: "active" | "failing" | "broken" = "active";
                      if (rawHealth === "broken" || (fetchCount >= 2 && successRate < 35)) {
                        healthStatusType = "broken";
                      } else if (rawHealth === "failing" || rawHealth === "unstable" || (fetchCount > 0 && successRate < 80)) {
                        healthStatusType = "failing";
                      } else {
                        healthStatusType = "active";
                      }
                      ...
                      const healthConfig = {
                        active: {
                          ...
                          Icon: CheckCircle2,
                          titleText: uiLanguage === "vi" 
                            ? `Hoạt động tốt: Tỉ lệ thành công ${successRate}% (${successCount}/${fetchCount})` 
                            : `Active & Healthy: ${successRate}% success rate (${successCount}/${fetchCount})`
                        },
                        failing: {
                          label: "Failing",
                          ...
                          Icon: AlertTriangle,
                          titleText: feed.healthError || (uiLanguage === "vi" 
                            ? `Chập chờn: Tỉ lệ thành công ${successRate}% (${successCount}/${fetchCount})` 
                            : `Failing / Unstable: ${successRate}% success rate (${successCount}/${fetchCount})`)
                        },
                        broken: {
                          label: "Broken",
                          color: colors.critical,
                          bgColor: `${colors.critical}18`,
                          borderColor: `${colors.critical}40`,
                          Icon: AlertCircle,
                          titleText: feed.healthError || (uiLanguage === "vi" 
                            ? "Nguồn tin bị hỏng: Không thể kết nối URL" 
                            : "Broken Feed: Failed to fetch RSS feed URL")
                        }
                      }[healthStatusType];
                        ...
                        >
                          <div className="min-w-0 flex-1 space-y-2">
                            <div className="flex flex-wrap items-center gap-2">
                              {/* Priority Starred badge */}
                              {feed.isStarred && (
                                <Badge 
                                  variant="outline" 
                                  className="text-[9px] font-bold px-1.5 py-0.5 uppercase flex items-center gap-1 transition-colors"
                                  style={{
                                    color: "#d97706",
                                ...
                                >
                                  ...
                                  <span>{uiLanguage === "vi" ? "Ưu tiên" : "Starred"}</span>
                                </Badge>
                              )}
                              ...
                              {/* Style/Type badge */}
                              {feed.feedType && (
                                <Badge variant="secondary" className="text-[9px] font-mono font-bold px-1.5 py-0.5 uppercase">
                                  {feed.feedType}
                                </Badge>
                              )}
                              ...
                              {/* Priority badge */}
                              <Badge 
                                variant={feed.priority === "high" ? "danger" : feed.priority === "low" ? "secondary" : "outline"}
                              ...
                              {/* Feed Health Indicator Badge */}
                              <Badge 
                                variant="outline"
                                className="text-[9px] font-extrabold px-2 py-0.5 rounded-full flex items-center gap-1.5 transition-colors cursor-help shadow-xs"
                                style={{
                                  color: healthConfig.color,
                                  borderColor: healthConfig.borderColor,
                                  backgroundColor: healthConfig.bgColor
                                }}
                                title={healthConfig.titleText}
                              >
                                <HealthIcon className="w-3 h-3 shrink-0" style={{ color: healthConfig.color }} />
                                <span className="uppercase tracking-wider">{healthConfig.label}</span>
                              </Badge>
                            </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap min-w-0">
                              ...
                              <FeedSparkline feed={feed} articles={activeArticles} uiLanguage={uiLanguage} />
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3 pt-0.5 text-[9px] font-mono font-bold" style={{ color: colors.textMuted }}>
                              <span className="flex items-center gap-1">
                                <Activity className="w-3 h-3 shrink-0" />
                                <span>{uiLanguage === "vi" ? `Lịch sử: ${successRate}% (${successCount}/${fetchCount})` : `Rate: ${successRate}% (${successCount}/${fetchCount})`}</span>
                              </span>
                              ...
                              {feed.lastFetchedAt ? (
                                <span className="text-[9px] flex items-center gap-1" style={{ color: colors.textMuted }}>
                                  • {uiLanguage === "vi" ? `Cập nhật: ${new Date(feed.lastFetchedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : `Synced: ${new Date(feed.lastFetchedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                                </span>
                              ) : null}
                              
                              <span className="flex items-center gap-1 text-teal-600 dark:text-teal-400 font-bold" title={uiLanguage === "vi" ? "Dữ liệu nguồn tin đã được lưu trữ offline" : "Feed data saved for offline reading"}>
                                • <CheckCircle2 className="w-2.5 h-2.5 shrink-0 text-teal-500" />
                                <span>{uiLanguage === "vi" ? "Đã đồng bộ Offline" : "Offline Synced"}</span>
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="flex items-center gap-1 justify-end shrink-0">
                      ...
                      <button
                        type="button"
                        onClick={() => handleToggleStarFeed(feed)}
                        className="p-2 rounded-app-lg transition-all border-0 hover:bg-opacity-80 flex items-center justify-center cursor-pointer"
                        style={{ 
                          color: feed.isStarred ? "#d97706" : colors.textMuted, 
                          backgroundColor: feed.isStarred ? "#f59e0b20" : colors.surfaceOverlay, 
                          minWidth: "36px", 
                          minHeight: "36px" 
                        }}
                        title={feed.isStarred ? (uiLanguage === "vi" ? "Bỏ ưu tiên (Unstar)" : "Unstar feed") : (uiLanguage === "vi" ? "Đánh dấu ưu tiên (Star)" : "Star feed")}
                      >
                        <Star className={cn("w-3.5 h-3.5", feed.isStarred && "fill-amber-500 text-amber-500")} />
                      </button>
      ...
      {(activeArticles.length > 0 || getSavedReadingList().length > 0) && (
        <RSSFeedList
          ...
          onAnalyzeSentiment={handleAnalyzeArticlesSentiment}
        ...
        <Card className="text-left space-y-4 animate-fade-in shadow-app-md" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 rounded-app-xl border space-y-3 text-left" style={{ backgroundColor: colors.surfaceOverlay, borderColor: colors.border }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t" style={{ borderColor: colors.border }}>
                <div className="space-y-1">
                  <label className="font-medium text-[10px] uppercase tracking-wider" style={{ color: colors.textMuted }}>
                    {uiLanguage === "vi" ? "Lọc Chủ đề:" : "Category Topic:"}
                  </label>
                  <select
                    value={briefCategory}
                    ...
                    style={{ backgroundColor: colors.surface, borderColor: colors.border, color: colors.textPrimary, minHeight: "36px" }}
                  >
                    <option value="All">{uiLanguage === "vi" ? "Tất cả chủ đề" : "All Categories"}</option>
                    {["Thời sự", "Công nghệ", "Kinh tế", "Giáo dục", "Sức khỏe", "Thể thao", "Giải trí", "Khác"].map(cat => (
                      <option key={cat} value={cat}>{cat}</option>
                    ))}
                  </select>
