...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-header-bg border-b border-border-subtle shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] min-h-[68px] h-auto flex flex-col justify-center py-2 sm:py-0">
      <div className="max-w-[1600px] mx-auto px-3 sm:px-6 flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 sm:gap-6">
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          {onMenuToggle && (
            <button 
              ...
              className="lg:hidden p-1.5 sm:p-2 text-text-muted hover:bg-surface-subtle rounded-xl transition-colors"
            >
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0 overflow-x-auto max-w-full w-full sm:w-auto justify-between sm:justify-end pb-2 sm:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {isOfflineMode && (
            <div 
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-500 text-[10px] font-bold uppercase tracking-wider font-mono animate-pulse shrink-0"
              title={uiLanguage === "vi" ? "Chế độ Ngoại tuyến: Đã giới hạn tải dữ liệu từ mạng, chỉ sử dụng bộ nhớ đệm local" : "Offline Mode: Network fetching restricted to cached local storage"}
            >
              <WifiOff className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">
                {uiLanguage === "vi" ? "Ngoại tuyến" : "Offline Mode"}
              </span>
            </div>
          )}

          {/* Real-time Sync Status & Manual Sync Shortcut */}
          <SyncStatus 
          ...
          <BatteryIndicator uiLanguage={uiLanguage} variant="header" />

          <div className="flex items-center gap-0.5">
            <button 
              onClick={onSearchClick}
              className="p-1.5 sm:p-2.5 text-text-muted hover:bg-surface-subtle hover:text-text-main rounded-xl transition-all group"
              title="Search (Ctrl+K)"
            >
            ...
            <button 
              ...
              className="relative p-1.5 sm:p-2.5 text-text-muted hover:bg-surface-subtle hover:text-text-main rounded-xl transition-all group"
              title="Notifications"
            >
            ...
            <button 
              ...
              className="hidden sm:block p-1.5 sm:p-2.5 text-text-muted hover:bg-surface-subtle hover:text-text-main rounded-xl transition-all group"
              title="Switch Theme"
            >
            ...
            <button 
              ...
              className="hidden sm:flex items-center gap-1.5 px-2 sm:px-3 py-1.5 text-text-muted hover:bg-surface-subtle hover:text-text-main rounded-xl transition-all group font-mono text-xs font-black uppercase tracking-wider"
          ...
          <div className="hidden sm:block w-px h-6 bg-border-subtle mx-1 sm:mx-2" />

          <button 
            onClick={onProfileClick}
            className="flex items-center gap-2 sm:gap-3 p-1 sm:p-1.5 hover:bg-surface-subtle rounded-xl transition-all group shrink-0"
          >
