...
import { exportBriefingAsWav, normalizeAudioBuffer } from "../utils/audioExport";
const AudioNormalizationDiagnostic = React.lazy(() => import("./AudioNormalizationDiagnostic").then(m => ({ default: m.AudioNormalizationDiagnostic })));
import { 
  Play, 
  Pause, 
  RotateCcw, 
...
import { NewsChapter, SummaryPayload, AudioPrebufferState } from "../types";
import { getPlayQueue } from "../features/store";
import { getEpisodeFromOffline } from "../services/offlineStorageService";
import { audioCrossfaderEngine } from "../utils/audioCrossfader";
...
// Helper: Remove DC offset to prevent clicks at start/end
function removeDcOffset(buffer: AudioBuffer) {
...
// Helper: Find nearest zero-crossing near the end of audio buffer to prevent DC step
function findZeroCrossingNearEnd(data: Float32Array, searchWindowSamples: number = 240): number {
...
// Helper: Apply Smooth Hann Equal-Power Crossfade and Anti-Noise Window to an AudioBuffer
function applySegmentCrossfade(
  buffer: AudioBuffer,
  fadeInMs: number = 25,
  fadeOutMs: number = 45
) {
  const sampleRate = buffer.sampleRate;
  const fadeInSamples = Math.min(Math.round((fadeInMs / 1000) * sampleRate), Math.floor(buffer.length / 2));
  const fadeOutSamples = Math.min(Math.round((fadeOutMs / 1000) * sampleRate), Math.floor(buffer.length / 2));

  for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
    const data = buffer.getChannelData(ch);

    // 1. Zero-crossing tail alignment to avoid trailing DC discontinuity
    const zeroCrossIdx = findZeroCrossingNearEnd(data, Math.min(480, Math.floor(buffer.length * 0.1)));
    if (zeroCrossIdx < data.length) {
      for (let j = zeroCrossIdx; j < data.length; j++) {
        data[j] = 0;
      }
    }

    // 2. Hann S-curve Fade-in on segment attack
    if (fadeInSamples > 0) {
    ...
    // 3. Smooth Cosine Fade-out on segment tail to eliminate end-of-segment noise bursts
    if (fadeOutSamples > 0) {
      const startIdx = Math.max(0, zeroCrossIdx - fadeOutSamples);
      const actualFadeLen = zeroCrossIdx - startIdx;
      if (actualFadeLen > 0) {
        for (let i = 0; i < actualFadeLen; i++) {
          const progress = (i + 1) / actualFadeLen;
...
// Helper: Resample AudioBuffer to unified targetSampleRate (24000Hz) using linear interpolation
function resampleAudioBuffer(audioCtx: AudioContext, buffer: AudioBuffer, targetSampleRate: number): AudioBuffer {
  if (buffer.sampleRate === targetSampleRate) return buffer;
  const ratio = buffer.sampleRate / targetSampleRate;
  const newLength = Math.round(buffer.length / ratio);
  const result = audioCtx.createBuffer(buffer.numberOfChannels, newLength, targetSampleRate);
  ...
  for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
    ...
    for (let i = 0; i < newLength; i++) {
      ...
      const index1 = Math.min(index0 + 1, srcData.length - 1);
      const t = srcIndex - index0;
      destData[i] = srcData[index0] * (1 - t) + srcData[index1] * t;
    }
  }
  return result;
}
...
export default function ManualPcmPlayer({ 
  ...
  const { speed } = useMotionDetection();
  ...
  const [sleepAtEnd, setSleepAtEnd] = useState<boolean>(false);
  const [showSleepMenu, setShowSleepMenu] = useState<boolean>(false);

  useEffect(() => {
    const checkTimer = () => {
      const left = localStorage.getItem("cc_sleep_seconds_left");
      const atEnd = localStorage.getItem("cc_sleep_at_briefing_end") === "true";
      setSleepSeconds(left ? Number(left) : null);
      setSleepAtEnd(atEnd);
    };
      ...
      if (leftStr) {
        ...
        if (left > 0) {
          ...
          if (newLeft === 0) {
            ...
            alert(uiLanguage === "vi" ? "Đã tạm dừng phát theo hẹn giờ ngủ" : "Sleep timer ended. Playback paused.");
          }
        } else {
          setSleepSeconds(null);
          localStorage.removeItem("cc_sleep_seconds_left");
  ...
  const [prebufferState, setPrebufferState] = useState<AudioPrebufferState>({
    chunkIndex: 0,
    totalChunks: audioChunks.length || 1,
    bufferedChunks: (audioChunks || []).map((_, i) => i),
    isPreloading: false,
    preloadProgressPct: 100
  });
  const hasPrefetchedNextRef = useRef<boolean>(false);
...
useEffect(() => {
  ...
  const initAudio = async () => {
    ...
    try {
      ...
      for (let i = 0; i < chunksToProcess.length; i++) {
        ...
        // Header-based routing to ensure native Web Audio API decoding
        let containerBuffer = arrayBuffer;
        ...
        if (!isWav && !isMp3) {
          // If raw PCM without container header, wrap with 24kHz WAV header
          const wavBlob = encodeWavHeader(arrayBuffer, 24000);
          containerBuffer = await wavBlob.arrayBuffer();
        }

        try {
          let decoded = await audioCtx.decodeAudioData(containerBuffer.slice(0));
          ...
          console.log(`[ManualPcmPlayer-AUDIT] ${chunkId} decoded natively in ${decodeDuration}ms. Rate: ${decoded.sampleRate}Hz, Duration: ${decoded.duration.toFixed(2)}s`);
          
        } catch (decodeErr) {
          const decodeDuration = Date.now() - decodeStart;
          console.warn(`[ManualPcmPlayer-AUDIT] ${chunkId} Native decoding failed after ${decodeDuration}ms. Falling back to raw PCM parser...`);
      ...
      // ===== UNIFIED RESAMPLING, RMS LOUDNESS NORMALIZATION, DC OFFSET REMOVAL & HANN CROSS-FADE SMOOTHING =====
      const targetSampleRate = 24000;
      const processedBuffers = decodedBuffers.map(buf => {
        const resampled = resampleAudioBuffer(audioCtx, buf, targetSampleRate);
        // Level RMS loudness to -18.4 dBFS target and soft-limit peaks
        normalizeAudioBuffer(resampled, { targetSampleRate, targetRms: 0.12, peakCeiling: 0.95 });
        // Apply 25ms Hann attack fade-in and 45ms cosine fade-out with zero-crossing alignment
        applySegmentCrossfade(resampled, 25, 45);
        return resampled;
      });

      const sampleRate = targetSampleRate;
      const numberOfChannels = Math.max(...processedBuffers.map(b => b.numberOfChannels));
      ...
      processedBuffers.forEach((buf, idx) => {
        ...
        if (idx < processedBuffers.length - 1) {
      ...
      audioCrossfaderEngine.initGraph(audioCtx);
      playAudio(0);
  ...
  const playAudio = (offset: number) => {
    ...
    // Smooth crossfade envelope on play start to eliminate sudden DC step clicks
    const startTime = audioCtx.currentTime;
    gainNode.gain.setValueAtTime(0.0001, startTime);
    gainNode.gain.linearRampToValueAtTime(normVolume, startTime + 0.020);
    ...
    const updateProgress = () => {
      ...
      if (activeIdx !== -1) {
        if (activeIdx !== activeSegmentIndexRef.current) {
          ...
          // Only trigger gentle stinger when background soundscape is enabled
          if (isBgMusicEnabled) {
            audioCrossfaderEngine.playTransitionChime();
          }
        }
      }
    ...
    if (sourceNodeRef.current) {
      ...
      try {
        if (gainNodeRef.current && audioCtxRef.current && audioCtxRef.current.state === "running") {
          ...
          gainNodeRef.current.gain.linearRampToValueAtTime(0.0001, now + 0.015);
        }
        setTimeout(() => {
          try {
            currentSource.stop();
  ...
  // Predictive Smart Audio Prebuffering for Upcoming Queue Items
  useEffect(() => {
    if (!isPlaying || totalDuration <= 0) return;

    // Trigger pre-buffering when playback reaches 60% of current track
    if (currentTime >= totalDuration * 0.6 && !hasPrefetchedNextRef.current) {
      hasPrefetchedNextRef.current = true;
      const queue = getPlayQueue();
      if (queue && queue.length > 0) {
        const nextItem = queue[0];
        if (nextItem && nextItem.id) {
          setPrebufferState(prev => ({ ...prev, isPreloading: true, preloadProgressPct: 20 }));
          getEpisodeFromOffline(nextItem.id)
            .then(cached => {
              if (cached && cached.audioChunks) {
                setPrebufferState(prev => ({
                  ...prev,
                  isPreloading: false,
                  preloadProgressPct: 100
                }));
                console.log(`[Smart Prebuffer] Successfully preloaded next queue track: ${nextItem.title}`);
              } else {
                setPrebufferState(prev => ({ ...prev, isPreloading: false, preloadProgressPct: 100 }));
              }
            })
            .catch(err => {
              console.warn("[Smart Prebuffer] Non-critical queue preload notice:", err);
              setPrebufferState(prev => ({ ...prev, isPreloading: false, preloadProgressPct: 100 }));
            });
        }
      }
  ...
  const handleRateChange = (rate: number) => {
    // Giới hạn rate từ 0.5x đến 2.5x
    if (rate < 0.4) {
      alert(uiLanguage === "vi" 
        ? "Tốc độ quá chậm. Khuyến nghị từ 0.5x đến 2.0x." 
        : "Speed too slow. Recommended 0.5x to 2.0x.");
      return;
    }
    if (rate > 2.5) {
      alert(uiLanguage === "vi" 
        ? "Tốc độ quá nhanh. Khuyến nghị từ 0.5x đến 2.0x." 
        : "Speed too fast. Recommended 0.5x to 2.0x.");
      return;
    }
    setPlaybackRate(rate);
    updatePreferences({ speed: rate });
  };
  ...
  const handleSeekTo = useCallback((targetTime: number) => {
    ...
    if (isPlaying) {
      ...
      elapsedOffsetRef.current = clamped;
      const activeIdx = segmentOffsets.findIndex(
        (offset) => clamped >= offset.start && clamped <= offset.end
      );
      if (activeIdx !== -1) {
        activeSegmentIndexRef.current = activeIdx;
        setActiveSegmentIndex(activeIdx);
  ...
  }, [totalDuration, isPlaying, playAudio, segmentOffsets]);

  // Media Session API Integration for Head Units / Android Auto / Apple CarPlay
  useEffect(() => {
    if (!('mediaSession' in navigator)) return;

    // Determine active segment chapter title for car display metadata
    const activeSeg = allSegments[activeSegmentIndex];
    const displayTitle = title || (uiLanguage === "vi" ? "Bản tin CommuteCast" : "CommuteCast Briefing");
    const displayArtist = activeSeg ? `${activeSeg.title} • CommuteCast Radio` : "CommuteCast Radio";

    // Update metadata with high & low resolution artwork matrix for car dashboards
    navigator.mediaSession.metadata = new MediaMetadata({
      title: displayTitle,
      artist: displayArtist,
      album: uiLanguage === "vi" ? "Bản tin phát thanh cá nhân hóa" : "Personalized Commute Briefing",
      artwork: [
        { src: '/icon-192.jpg', sizes: '96x96', type: 'image/jpeg' },
        { src: '/icon-192.jpg', sizes: '128x128', type: 'image/jpeg' },
        { src: '/icon-192.jpg', sizes: '192x192', type: 'image/jpeg' },
        { src: '/icon-512.jpg', sizes: '256x256', type: 'image/jpeg' },
        { src: '/icon-512.jpg', sizes: '384x384', type: 'image/jpeg' },
    ...
    // Set action handlers for car touchscreens & steering wheel controls
  ...
  }, [isPlaying, title, activeSegmentIndex, allSegments, uiLanguage, onEnded, handlePlayPause, handleSkip, handleSeekTo, currentTime]);

  // Synchronize Live Position State & Send Service Worker Keep-Alive to Car Head Unit
  useEffect(() => {
    if (!('mediaSession' in navigator) || !('setPositionState' in navigator.mediaSession)) return;
    if (totalDuration > 0 && Number.isFinite(totalDuration) && Number.isFinite(currentTime)) {
      try {
        const safePosition = Math.max(0, Math.min(currentTime, totalDuration));
  ...
  // Background Audio Service Worker Keep-Alive Ping during Driving Session
  useEffect(() => {
    if (!isPlaying || !('serviceWorker' in navigator)) return;

    const pingInterval = setInterval(() => {
      if (navigator.serviceWorker.controller) {
        navigator.serviceWorker.controller.postMessage({
          type: "BACKGROUND_AUDIO_KEEPALIVE",
          payload: {
            title: title || "CommuteCast Active Drive Session",
            currentTime,
            activeSegmentIndex
          }
        });
  ...
  }, [isPlaying, title, currentTime, activeSegmentIndex]);
          ...
          <div className="relative border rounded-[2rem] p-8 shadow-2xl overflow-hidden"
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div className="flex items-center gap-3">
                ...
                {prebufferState.preloadProgressPct === 100 && (
                  <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[9px] font-black uppercase tracking-widest bg-emerald-500/10 border-emerald-500/20 text-emerald-400 font-mono">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                    <span>0ms Prebuffer</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  title={uiLanguage === "vi" ? "Chẩn đoán âm lượng RMS" : "RMS Loudness Diagnostic"}
                >
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="relative">
                  <button
                    ...
                    className={cn(
                      "h-9 flex items-center gap-2 px-3 rounded-2xl border text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-2xs"
                    )}
                    style={(sleepSeconds !== null || sleepAtEnd) 
                      ? { backgroundColor: `${colors.interactive}1a`, borderColor: colors.interactive, color: colors.interactive } 
                      : { backgroundColor: `${colors.surface}80`, borderColor: colors.border, color: colors.textMuted }}
                    title={uiLanguage === "vi" ? "Thiết lập hẹn giờ ngủ" : "Set Sleep Timer"}
                  >
                    <Moon className="w-3.5 h-3.5" />
                    {sleepSeconds !== null ? (
                      <span className="font-mono">
                        {Math.floor(sleepSeconds / 60)}:{String(sleepSeconds % 60).padStart(2, "0")}
                      </span>
                    ) : sleepAtEnd ? (
                      <span>{uiLanguage === "vi" ? "Hết bài" : "End"}</span>
                    ) : (
                      <span>{uiLanguage === "vi" ? "Hẹn giờ ngủ" : "Sleep"}</span>
                    )}
                    <ChevronDown className="w-3 h-3 opacity-60" />
                  </button>

                  {showSleepMenu && (
                    <div className="absolute right-0 bottom-full mb-2 w-44 border rounded-app-xl shadow-2xl z-[100] py-1 text-[10px] font-black uppercase tracking-wider"
                      ...
                      {[15, 30, 60].map((m) => (
                        <button 
                          ...
                          type="button"
                          onClick={() => {
                            localStorage.setItem("cc_sleep_seconds_left", String(m * 60));
                            localStorage.removeItem("cc_sleep_at_briefing_end");
                            setSleepSeconds(m * 60);
                            setSleepAtEnd(false);
                            setShowSleepMenu(false);
                          }} 
                          className="w-full text-left px-4 py-2 hover:bg-amber-500/10 hover:text-amber-500 transition-colors cursor-pointer"
                          style={{ color: colors.textPrimary, minHeight: "32px" }}
                        >
                          {m} {uiLanguage === "vi" ? "phút" : "minutes"}
                        </button>
                      ))}
                      <button 
                        type="button"
                        ...
                        className="w-full text-left px-4 py-2 border-t hover:bg-amber-500/10 hover:text-amber-500 transition-colors cursor-pointer"
                        style={{ color: colors.interactive, borderColor: colors.border, minHeight: "32px" }}
                      >
                        {uiLanguage === "vi" ? "Hết bản tin" : "End of briefing"}
                      </button>
                      <button 
                        type="button"
                        onClick={() => {
                        ...
                        className="w-full text-left px-4 py-2 border-t hover:bg-rose-500/10 hover:text-rose-500 transition-colors cursor-pointer"
                        style={{ color: colors.critical, borderColor: colors.border, minHeight: "32px" }}
                      >
                        {uiLanguage === "vi" ? "Tắt hẹn giờ" : "Turn off timer"}
                      </button>
                    </div>
                  )}
                </div>
          ...
          <Card className="p-6 shadow-sm flex flex-col gap-6"
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button 
                variant="outline"
                onClick={() => setShowDiagnosticModal(true)}
                className="h-auto py-4 flex flex-col gap-2 rounded-2xl border-amber-500/30 hover:border-amber-500/60 transition-all group"
                style={{ backgroundColor: `${colors.surface}` }}
              >
                <Activity className="w-5 h-5 text-amber-500 group-hover:scale-110 transition-transform" />
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-500">
                  {uiLanguage === "vi" ? "Chẩn Đoán RMS" : "RMS Diagnostic"}
                </span>
              </Button>
              <Button 
                variant="outline"
