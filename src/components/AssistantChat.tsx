              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {messages.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    ...
                    {isListening && (
                      <div className="flex flex-row-reverse gap-3 items-center animate-fade-in">
                        <div
                          className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-150"
                          style={{
                            backgroundColor: colors.critical,
                            color: colors.onCritical,
                            transform: `scale(${1 + (audioLevel || 0) * 0.35})`,
                            boxShadow: `0 0 16px ${colors.critical}66`
                          }}
                        >
                          <Mic className="w-4 h-4 animate-pulse" />
                        </div>
                        <div className="flex flex-col items-end gap-0.5 max-w-[80%]">
                          <span className="text-xs font-black uppercase tracking-widest" style={{ color: colors.critical }}>
                            {t.listening}
                          </span>
                          {interimTranscript && (
                            <span className="text-[11px] font-medium italic text-right truncate max-w-[220px]" style={{ color: colors.textSecondary }}>
                              {interimTranscript}
                            </span>
                          )}
                        </div>
                    ...
                    {isAiTranscribing && (
                      <div className="flex flex-row-reverse gap-3 items-center animate-fade-in">
                        <div
                          className="w-9 h-9 rounded-full flex items-center justify-center animate-spin"
                          style={{ backgroundColor: `${colors.interactive}22`, color: colors.interactive }}
                        >
                          <Sparkles className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col items-end gap-0.5">
                          <span className="text-xs font-bold" style={{ color: colors.interactive }}>
                            {interimTranscript || (uiLanguage === "vi" ? "Đang xử lý âm thanh AI..." : "Transcribing with AI...")}
                          </span>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="flex-1 border rounded-2xl transition-all focus-within:ring-1"
                    ...
                    <div className="flex items-center justify-between px-3 pb-2">
                      <button
                        ...
                        disabled={isAiTranscribing || isProcessing}
                        title={isListening ? (uiLanguage === "vi" ? "Dừng ghi âm & gửi" : "Stop & send") : (uiLanguage === "vi" ? "Nói với trợ lý" : "Speak to assistant")}
                        className="p-2 rounded-xl transition-all flex items-center gap-1.5"
                        style={isListening 
                          ? { backgroundColor: `${colors.critical}22`, color: colors.critical, border: `1px solid ${colors.critical}44` } 
                          : isAiTranscribing
                          ? { backgroundColor: `${colors.interactive}22`, color: colors.interactive }
                          : { color: colors.textMuted }}
                      >
                        <Mic className={cn("w-4 h-4", isListening && "animate-pulse")} />
                        {isListening && (
                          <span className="text-[10px] font-black uppercase tracking-wider">
                            {uiLanguage === "vi" ? "Đang nghe" : "Listening"}
                          </span>
                        )}
                      </button>
                      <button
                        type="button"
                        onClick={handleSend}
                        disabled={isProcessing || isAiTranscribing || !inputVal.trim()}
                        className="p-1.5 rounded-lg transition-all"
                        style={inputVal.trim() && !isProcessing && !isAiTranscribing
                          ? { backgroundColor: `${colors.interactive}1a`, color: colors.interactive } 
