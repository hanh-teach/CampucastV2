...
import { isValidPublicUrl, sanitizeStringInput } from "../utils/security";
...
router.get("/parse-rss", async (req, res): Promise<any> => {
  ...
  const sanitizedUrl = sanitizeStringInput(url, 2048);
  if (!isValidPublicUrl(sanitizedUrl)) {
    return res.status(400).json({ error: "URL không hợp lệ hoặc nằm trong dải truy cập bị hạn chế (SSRF protection)." });
  }
...
Length: roughly 300-400 words. Write the article content directly, with no extra conversational preambles or notes.${aiMode ? `\nStyle/Approach preference: Mode ${aiMode}.` : ""}`;
    ...
    if (hasGroq) {
      try {
        console.log("[CommuteCast] Groq API setup detected for News Generation. Running Groq...");
        ...
        console.warn("[CommuteCast] Groq news generation failed, falling back to Gemini:", groqErr?.message || groqErr);
        newsText = "";
      }
    }
...
// Auto-Categorize RSS Feeds via Gemini AI
router.post("/auto-categorize-feeds", async (req, res): Promise<any> => {
  const { feeds, language } = req.body;
  const isVi = language === "vi" || language === "bilingual";
  ...
  try {
    const feedsToClassify = feeds.map((f: any) => ({
      id: f.id,
      title: f.title || "",
      url: f.url || "",
    ...
    const prompt = `Bạn là một chuyên gia phân tích và phân loại nguồn tin tức RSS.
Dưới đây là danh sách các nguồn tin RSS (bao gồm ID, Tiêu đề channel, URL và danh mục hiện tại):
${JSON.stringify(feedsToClassify, null, 2)}

Nhiệm vụ của bạn: Hãy phân tích kỹ tiêu đề channel và URL của từng nguồn tin RSS, sau đó gán danh mục chuẩn xác nhất cho từng nguồn tin.
Danh mục BẮT BUỘC phải là một trong các giá trị sau:
${allowedCategories.map(c => `- ${c}`).join("\n")}

Trả về mảng JSON đúng cấu trúc schema được yêu cầu.`;
    ...
    let categorized: Array<{ id: string; suggestedCategory: string; reason?: string }> = [];
    
    try {
      categorized = JSON.parse(jsonText);
    } catch (parseErr) {
      console.warn("Error parsing Gemini json response for feed auto-categorize:", parseErr);
    }

    return res.json({ categorized });
  } catch (error: any) {
    console.error("Auto-categorize feeds error:", error);
    const friendlyError = parseGeminiError(error, isVi, false);
    return res.status(500).json({ error: friendlyError });
  }
});

// Analyze Sentiment for RSS Articles via Gemini AI
router.post("/analyze-rss-sentiment", async (req, res): Promise<any> => {
  const { articles, language } = req.body;
  const isVi = language === "vi" || language === "bilingual";
  ...
  try {
    const articlesToAnalyze = articles.map((art: any, index: number) => ({
      index,
      title: art.title || "",
      content: (art.content || art.description || "").slice(0, 300)
    }));

    const prompt = `Bạn là chuyên gia phân tích cảm xúc (Sentiment Analysis) cho báo chí và tin tức truyền thông.
Dưới đây là danh sách các bài viết (chỉ số index, tiêu đề, tóm tắt):
${JSON.stringify(articlesToAnalyze, null, 2)}

Nhiệm vụ: Hãy phân tích cảm xúc chính của từng bài viết và gán một trong 3 nhãn cảm xúc duy nhất:
- "positive" (Tích cực: thành tựu, phát triển, tin vui, thắng lợi, tăng trưởng, hòa bình, phát minh, vinh danh...)
- "negative" (Tiêu cực: tai nạn, dịch bệnh, lừa đảo, khủng hoảng, lỗ, sụp đổ, tranh cãi, thiên tai, xung đột, tử vong...)
- "neutral" (Trung lập: đưa tin khách quan, số liệu báo cáo, thông báo quy định, sự kiện bình thường, hướng dẫn...)
...
Trả về mảng JSON đúng cấu trúc schema được yêu cầu.`;
    ...
    const jsonText = response.text ? response.text.trim() : "[]";
    let results: Array<{ index: number; sentiment: string }> = [];
    
    try {
      results = JSON.parse(jsonText);
    } catch (parseErr) {
      console.warn("Error parsing Gemini json response for sentiment analysis:", parseErr);
    }

    return res.json({ results });
  } catch (error: any) {
    console.error("Analyze RSS sentiment error:", error);
    const friendlyError = parseGeminiError(error, isVi, false);
    return res.status(500).json({ error: friendlyError });
  }
});
