...
router.post("/assistant-chat", async (req, res): Promise<any> => {
  const { history = [], message, language, userPreferences = [], memoryContext = "" } = req.body;
  const isVi = language === "vi" || language === "vi-VN" || language === "bilingual";
...
USER PREFERENCE & PERSONAL AI MEMORY CONTEXT:
The user has expressed strong interest in these topics based on their offline commute news reading history:
${userPreferencesText}

${memoryContext ? `${memoryContext}\n` : ""}
Proactively recommend news or direct subjects aligned with these interests and memory facts if the user asks for suggestions or recommendations (e.g., "What should I read today?", "Tôi nên đọc gì hôm nay?", "Recommend news", "Give me something interesting").
...
// ===== DUAL-ENGINE AI AUDIO TRANSCRIBE ENDPOINT =====
// Accepts audio blob Base64 from MediaRecorder and uses Gemini AI Speech Transcription
// Guaranteed cross-browser fallback when browser WebSpeech API fails (Chrome network errors, permission blocks)
const handleAudioTranscribe = async (req: express.Request, res: express.Response): Promise<any> => {
  const { audioBase64, mimeType, language } = req.body;
  const isVi = language === "vi" || language === "vi-VN" || language === "bilingual" || !language;

  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).json({
      error: isVi ? "Chưa cấu hình GEMINI_API_KEY trên server." : "GEMINI_API_KEY is not configured on the server."
    });
  }

  if (!audioBase64 || typeof audioBase64 !== "string") {
    return res.status(400).json({
      error: isVi ? "Không nhận được dữ liệu âm thanh để phiên âm." : "No audio data provided for transcription."
    });
  }

  try {
    // Sanitize base64 payload (strip data:audio/...;base64, if present)
    ...
    // Sanitize MIME type for Gemini Multimodal Audio
    let cleanMimeType = "audio/webm";
    if (mimeType && typeof mimeType === "string") {
      const lower = mimeType.toLowerCase();
      if (lower.includes("webm")) cleanMimeType = "audio/webm";
      else if (lower.includes("wav") || lower.includes("wave")) cleanMimeType = "audio/wav";
      else if (lower.includes("ogg")) cleanMimeType = "audio/ogg";
      else if (lower.includes("mp4") || lower.includes("m4a") || lower.includes("aac")) cleanMimeType = "audio/mp4";
      else if (lower.includes("mp3") || lower.includes("mpeg")) cleanMimeType = "audio/mp3";
    }

    const systemPrompt = `You are a high-precision speech-to-text transcriber for CommuteCast Enterprise broadcast assistant.
Listen carefully to the provided audio recording.
Your task: Transcribe the user's spoken words into text verbatim.

CRITICAL TRANSCRIPTION RULES:
1. Accurately transcribe in the language spoken (Vietnamese, English, etc.).
2. If spoken in Vietnamese, ensure 100% accurate Vietnamese standard spelling, tone marks (dấu tiếng Việt), acronyms, and proper nouns.
3. If the audio contains only background silence, white noise, mic clicks, or unintelligible murmurs with no human speech, return empty string "".
4. Output MUST be ONLY a JSON object with this exact schema:
{
  "transcript": "string"
}
Do NOT output markdown blocks, commentary, or explanation outside of the JSON object.`;

    const userPrompt = `Transcribe the attached audio recording. Preferred user language: ${language || "vi"}.`;
    ...
    if (response && response.text) {
      try {
        ...
        // Fallback cleanup if model outputs text directly
        transcript = response.text.replace(/```json/gi, "").replace(/```/gi, "").trim();
        try {
          const secondParse = JSON.parse(transcript);
          transcript = secondParse.transcript || "";
        } catch {
          // Use raw text cleaned
          transcript = transcript.replace(/^\{.*"transcript"\s*:\s*"/, "").replace(/"\s*\}$/, "").trim();
        }
      }
    }
...
router.post("/assistant/transcribe", handleAudioTranscribe);
