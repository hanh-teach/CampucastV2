...
const EDGE_VOICE_MAP: Record<string, string> = {
  "vi-HN-male": "vi-VN-NamMinhNeural",   // MC Minh (Male Northern - 2024)
  "vi-HN-female": "vi-VN-HoaiMyNeural",  // Female Northern
  "vi-HN": "vi-VN-NamMinhNeural",        // Default Northern -> Male
  "vi-HCM-female": "vi-VN-HoaiMyNeural", // MC An (Female Southern)
  "vi-HCM-male": "vi-VN-NamMinhNeural",   // Male Southern
  "vi-HCM": "vi-VN-HoaiMyNeural",        // Default Southern -> Female
  "vi": "vi-VN-NamMinhNeural",           // Default Vietnamese
...
async function callGeminiTTSForChunk(chunk: string, voice: string, tone: string, emotion?: string): Promise<string> {
  ...
  } else if (voice === "vi-HN" || voice === "vi-HN-male" || voice === "vi") {
    voiceName = "Kore";
    systemInstructionText = `ROLE & STYLE: Bạn là MC Minh (Nam - Giọng miền Bắc), phát thanh viên tin tức nam truyền hình sắc bén, mạch lạc và ấm áp...`;
  } else if (voice === "vi-HCM" || voice === "vi-HCM-female") {
    voiceName = "Zephyr";
    systemInstructionText = `ROLE & STYLE: Bạn là MC An (Nữ - Giọng miền Nam), đồng dẫn chương trình podcast duyên dáng, gần gũi và sinh động...`;
...
async function synthesizeSingleChunk(chunk: string, preferredVoice: string, tone: string, emotion?: string, preSegmentedLang?: "vi" | "en"): Promise<Buffer> {
  ...
  if (!success || !base64Audio) return encodeWavHeaderNode(Buffer.alloc(48000, 0), 24000);
  const rawBuf = Buffer.from(base64Audio, "base64");
  return wrapAsWavIfRawPcm(rawBuf);
}
...
router.post("/tts", async (req, res): Promise<any> => {
  ...
  try {
    ...
    const { buffer: mergedBuffer } = joinAudioBuffersAuto(validBuffers, 24000);
...
router.get("/music-preview/:type", (req, res) => {
  ...
  const safeType = path.basename(type).replace(/[^a-zA-Z0-9_-]/g, "");
  if (!safeType) {
    return res.status(400).json({ success: false, error: "Invalid audio type parameter" });
  }

  const possiblePaths = [
    path.join(process.cwd(), "public", `${safeType}.mp3`),
    path.join(process.cwd(), "public", `${safeType}.wav`),
    path.join(process.cwd(), "public", "music", `${safeType}.mp3`),
    path.join(process.cwd(), "public", "music", `${safeType}.wav`),
    path.join(process.cwd(), "assets", `${safeType}.mp3`),
    path.join(process.cwd(), "assets", `${safeType}.wav`),
  ];
