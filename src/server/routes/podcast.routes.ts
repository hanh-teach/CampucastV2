...
router.get("/local-podcasts/:filename", (req, res) => {
  ...
  const filePath = path.join(LOCAL_AUDIO_DIR, safeFilename);

  const resolvedDir = path.resolve(LOCAL_AUDIO_DIR);
  const resolvedPath = path.resolve(filePath);

  if (!resolvedPath.startsWith(resolvedDir) || !fs.existsSync(filePath)) {
...
router.post("/podcast/publish", async (req, res): Promise<any> => {
  try {
    ...
    const { buffer: finalAudioBuffer, contentType, fileExt } = joinAudioBuffersAuto(rawBuffers, 24000);
    const isMp3 = contentType === "audio/mpeg";
    ...
    if (isMp3) {
      calculatedDuration = Math.round(getMp3Duration(finalAudioBuffer));
    } else {
      calculatedDuration = Math.round(finalAudioBuffer.length / 48000);
    }
