...
### `POST /api/assistant/transcribe` (and `/api/transcribe`)
- **Purpose**: Multimodal audio transcription endpoint providing cross-browser fallback for speech recognition (Chrome failover).
- **Payload**: `{ audioBase64: string, mimeType?: string, language?: string }`
- **Response**: `{ success: boolean, transcript: string }`
- **Behavior**: Uses Gemini 3.7 Flash server-side with strict JSON schema to transcribe Vietnamese and English voice inputs verbatim with full diacritic accuracy.
